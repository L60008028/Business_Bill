    ...
    public class BnetInfoDAL
    {
        ...
        private BnetInfoModel FillData(SqlDataReader sdr)
        {
            ...
            try
            {
                ...
                model.TaoCanType = sdr.GetString(4); 
        ...
        {
            string sql = string.Format("select BnetId,BnetAccount,Price ,AttributeId from jy15.gdkltx.dbo.t_bnetinfo where BnetId={0} and BnetAccount={1}", bnetId, bnetAccount);
        ...
        public Dictionary<string, BnetInfoModel> GetAll()
        {
            ...
            string sql = "  select BnetId,BnetAccount,Price,AttributeId,TaoCanType from jy15.gdkltx.dbo.t_bnetinfo";
            using (SqlDataReader sdr = SqlHelper.ExecuteReader(lp.SqlConnStr, CommandType.Text, sql, null))
            {
                while (sdr.Read())
                {
